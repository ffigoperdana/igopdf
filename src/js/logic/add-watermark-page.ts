...
function loadPageConfig(pageNum: number) {
  ...
  if (config.type === 'text') {
    typeTextBtn!.className =
      'flex-1 py-2 px-3 text-sm font-medium rounded-lg bg-palm-600 text-white transition-colors';
    typeImageBtn!.className =
      'flex-1 py-2 px-3 text-sm font-medium rounded-lg bg-surface-muted text-content-muted hover:bg-surface-muted transition-colors';
...
function updatePresetHighlight(x: number, y: number) {
  document.querySelectorAll('.pos-preset-btn').forEach((btn) => {
    ...
    if (Math.abs(bx - x) < 0.01 && Math.abs(by - y) < 0.01) {
      btn.className =
        'pos-preset-btn py-1.5 text-xs bg-palm-600 hover:bg-palm-500 text-white rounded-md transition-colors';
    } else {
      btn.className =
        'pos-preset-btn py-1.5 text-xs bg-surface-muted hover:bg-surface-muted text-content-muted rounded-md transition-colors';
    }
  });
}
