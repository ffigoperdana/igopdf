...
type DocxRoutingInput = {
  serverEnabled: boolean;
  fileCount: number;
  mode: ServerDocxMode;
  fileSize: number;
  clientMaxBytes: number;
  textLayerKind?: TextLayerKind | null;
};
...
export function shouldUseServerDocxConversion({
  serverEnabled,
  fileCount,
  mode,
  fileSize,
  clientMaxBytes,
  textLayerKind,
}: DocxRoutingInput): boolean {
  const canUseClientEditableConversion =
    mode === 'editable' &&
    fileSize <= clientMaxBytes &&
    textLayerKind === 'native';

  // Preserve the existing browser ZIP flow for an eligible editable batch.
  // The server currently accepts only one PDF per conversion job.
  if (fileCount > 1 && canUseClientEditableConversion) return false;

  // The improved server converter repairs spacing and ND layout. Prefer it for
  // every single-file job while it is available, including small native PDFs.
  if (fileCount === 1 && serverEnabled) return true;

  // With the server disabled, retain the small/native browser fallback. Modes
  // that inherently need the server continue to surface the unavailable error.
  return !canUseClientEditableConversion;
}
...
function knownStructureError(error: unknown): boolean {
  ...
  return /object out of range|xref|extract_font|FzErrorFormat|font/i.test(
    message
  );
}
...
function friendlyServerError(code: string | null): string {
  switch (code) {
    case 'INVALID_PDF_STRUCTURE':
      return translated(
        'pdfToWord.errors.invalidStructure',
        'This PDF has an invalid internal structure. The repair attempt could not make it convertible.'
      );
    case 'FONT_OR_LAYOUT_UNSUPPORTED':
      return translated(
        'pdfToWord.errors.fontLayout',
        'This PDF uses embedded fonts or layout objects that could not be reconstructed as editable Word content. Try Preserve appearance.'
      );
    case 'ENCRYPTED_PDF':
      return translated(
        'pdfToWord.errors.encrypted',
        'Remove the PDF password first, then retry conversion.'
      );
    case 'PAGE_LIMIT':
      return translated(
        'pdfToWord.errors.pageLimit',
        'This conversion mode reached its page limit for the current server capacity.'
      );
    case 'MEMORY_LIMIT':
      return translated(
        'pdfToWord.errors.memoryLimit',
        'This document is too image-heavy for the current server capacity. Try fewer pages or Preserve appearance.'
      );
    case 'TIMEOUT':
      return translated(
        'pdfToWord.errors.timeout',
        'The conversion reached its processing time limit.'
      );
    case 'CANCELLED':
      return translated(
        'pdfToWord.errors.cancelled',
        'The conversion was cancelled.'
      );
    default:
      return translated(
        'pdfToWord.errors.general',
        'The server could not convert this PDF. Please retry once; if the issue continues, contact the administrator.'
      );
  }
}
...
async function inspectTextLayer(
  ...
  const kind: TextLayerKind =
    imageBasedPages === 0
      ? 'native'
      : imageBasedPages === samplePages
        ? 'image'
        : 'mixed';
...
document.addEventListener('DOMContentLoaded', () => {
  void (async () => {
    ...
    const qualityMessage = document.getElementById(
      'conversion-quality-message'
    );
    ...
    const serverProgress = document.getElementById(
      'server-conversion-progress'
    ) as HTMLProgressElement;
    const cancelServerButton = document.getElementById(
    ...
    const firstProfile = (): PdfTextLayerProfile | null =>
      state.files[0] ? profiles.get(state.files[0]) || null : null;
    ...
    const updateModeCopy = () => {
      ...
      const help =
        selected === 'auto'
          ? profile?.kind === 'image'
            ? translated(
                'pdfToWord.mode.autoImage',
                'Image-based PDF detected. OCR editable text is recommended.'
              )
            : profile?.kind === 'mixed'
              ? translated(
                  'pdfToWord.mode.autoMixed',
                  'Mixed PDF detected. OCR can recover image-only pages, but layout may change.'
                )
              : serverConfig.enabled
                ? translated(
                    'pdfToWord.mode.autoNativeServer',
                    'Native text PDF detected. The server will reconstruct editable text and repair document spacing.'
                  )
                : translated(
                    'pdfToWord.mode.autoNative',
                    'Native text PDF detected. The browser will try editable conversion first.'
                  )
          : mode === 'ocr'
            ? translated(
                'pdfToWord.mode.ocr',
                'OCR makes scanned text editable, but words and layout can differ from the PDF.'
              )
            : mode === 'visual'
              ? translated(
                  'pdfToWord.mode.visual',
                  'Each PDF page is placed as an image in Word. Appearance is preserved but text is not editable.'
                )
              : translated(
                  'pdfToWord.mode.editable',
                  'Reconstruct editable Word text. Complex tables, slide layouts, and fonts may change.'
                );
      if (modeHelp) modeHelp.textContent = help;
      if (privacyHint)
        privacyHint.textContent = serverConfig.enabled
          ? translated(
              'pdfToWord.privacy.serverPreferred',
              'Single PDFs use a private temporary server job for higher-quality conversion. Eligible batches stay in your browser, and server files are deleted after download or expiry.'
            )
          : translated(
              'pdfToWord.privacy.browser',
              'Files are processed in your browser. OCR and server repair are currently unavailable.'
            );
    };

    const updateQualityNotice = () => {
      if (!qualityNotice || !qualityMessage) return;
      const allProfiles = state.files
        .map((file) => profiles.get(file))
        .filter((profile): profile is PdfTextLayerProfile => Boolean(profile));
      if (allProfiles.length === 0) {
        qualityNotice.classList.add('hidden');
        return;
      }
      const imagePages = allProfiles.reduce(
        (total, profile) => total + profile.imageBasedPages,
        0
      );
      const sampledPages = allProfiles.reduce(
      ...
      qualityMessage.textContent =
        imagePages > 0
          ? translated(
              'pdfToWord.imageBasedDetected',
              `${imagePages} of ${sampledPages} sampled pages do not have usable text. OCR is recommended for scanned or image-based PDFs.`,
              { imagePages, samplePages: sampledPages }
            )
          : translated(
              'pdfToWord.nativeTextDetected',
              'Usable text was found. Font, table, spacing, and complex layout can still change in DOCX.'
            );
      qualityNotice.classList.remove('hidden');
      if (openOcrLink) {
        openOcrLink.textContent = translated(
          'pdfToWord.openOcr',
          'Open OCR PDF'
        );
      ...
      for (let index = 0; index < state.files.length; index++) {
        ...
        remove.onclick = () => {
          state.files = state.files.filter(
            (_: File, itemIndex: number) => itemIndex !== index
          );
          void updateUi();
        };
    ...
    const validateForMode = (
      file: File,
      mode: ServerDocxMode
    ): string | null => {
      if (file.size > serverConfig.maxUploadBytes) {
        return translated(
          'pdfToWord.errors.fileLimit',
          'PDF to Word currently supports a maximum of 50 MB per file on this server.'
        );
      }
      const profile = profiles.get(file);
      const maxPages =
        mode === 'ocr'
          ? serverConfig.ocrMaxPages
          : mode === 'visual'
            ? serverConfig.visualMaxPages
            : serverConfig.nativeMaxPages;
      if (profile && profile.pageCount > maxPages) {
        return translated(
          'pdfToWord.errors.pageLimit',
          `This conversion mode supports up to ${maxPages} pages per PDF.`,
          { limit: maxPages }
        );
      }
      return null;
    ...
    const fetchSlot = async (slotId: string): Promise<UploadSlot> => {
      const response = await fetch(`/api/docx/upload-slots/${slotId}`, {
        credentials: 'include',
        cache: 'no-store',
      });
    ...
    const uploadServerFile = async (
      ...
      const uploadOnce = (current: UploadSlot): Promise<string> =>
        new Promise((resolve, reject) => {
          const upload = new TusUpload(file, {
            endpoint: '/api/docx/uploads',
            uploadUrl:
              current.status === 'uploading'
                ? `/api/docx/uploads/${current.id}`
                : null,
            ...
            onProgress: (sent, total) => {
              if (!cancelRequested && total > 0)
                setServerStatus(
                  translated(
                    'pdfToWord.status.uploading',
    ...
    const stageMessage = (
      ...
      if (job.status === 'queued')
        return queuePosition && queuePosition > 1
          ? translated(
              'pdfToWord.status.queue',
              `Waiting in the server queue. Position ${queuePosition}.`,
              { position: queuePosition }
            )
          : translated(
              'pdfToWord.status.queued',
      ...
      const base: Record<string, string> = {
        validating: translated(
          'pdfToWord.status.validating',
          'Validating PDF...'
        ),
        analyzing: translated(
          'pdfToWord.status.analyzing',
          'Analyzing document...'
        ),
        repairing: translated(
          'pdfToWord.status.repairing',
          'Repairing PDF structure...'
        ),
        converting: translated(
          'pdfToWord.status.converting',
          'Reconstructing editable DOCX...'
        ),
        ocr: translated('pdfToWord.status.ocr', 'Recognizing text with OCR...'),
        rendering: translated(
          'pdfToWord.status.rendering',
          'Rendering pages into DOCX...'
        ),
        packaging: translated(
          'pdfToWord.status.packaging',
          'Packaging DOCX...'
        ),
      };
    ...
    const downloadServerResult = async (jobId: string, file: File) => {
      const response = await fetch(`/api/docx/jobs/${jobId}/download`, {
        credentials: 'include',
      });
      if (!response.ok) throw new Error(await readApiError(response));
      downloadFile(
    ...
    const runServerConversion = async (file: File, mode: ServerDocxMode) => {
      ...
      setServerStatus(
        translated(
          'pdfToWord.status.preparing',
          'Preparing secure server upload...'
        ),
        0,
        true
      );
      ...
      while (!cancelRequested) {
        const response = await fetch(`/api/docx/jobs/${jobId}`, {
          credentials: 'include',
          cache: 'no-store',
        });
        if (!response.ok) throw new Error(await readApiError(response));
        const payload = (await response.json()) as {
          data?: { job?: ServerJob; queuePosition?: number | null };
        };
        ...
        if (job.status === 'completed') {
          ...
          activeJobId = null;
          showAlert(
            translated('pdfToWord.completeTitle', 'Conversion complete'),
            translated(
              'pdfToWord.completeServer',
              'DOCX downloaded. Temporary server files were removed.'
            ),
            'success',
            resetState
          );
        ...
        if (job.status === 'failed')
          throw new Error(friendlyServerError(job.errorCode));
        if (job.status === 'cancelled') throw new Error('CANCELLED');
        setServerStatus(
          stageMessage(job, payload.data?.queuePosition ?? null),
          job.progress,
          true
        );
    ...
    const runClientConversion = async (files: File[]) => {
      showLoader(
        translated('pdfToWord.status.loading', 'Loading PDF converter...')
      );
      ...
      if (state.files.length === 1) {
        ...
        showLoader(
          translated(
            'pdfToWord.status.clientConverting',
        ...
        for (let index = 0; index < state.files.length; index++) {
          ...
          showLoader(
            translated(
              'pdfToWord.status.clientBatch',
              `Converting ${index + 1}/${state.files.length}: ${file.name}...`,
              { index: index + 1, total: state.files.length, name: file.name }
            )
          );
          const docx = await pymupdf.pdfToDocx(file);
          zip.file(
      ...
      showAlert(
        translated('pdfToWord.completeTitle', 'Conversion complete'),
        translated(
          'pdfToWord.completeClient',
          'Review the DOCX for fonts, tables, spacing, and complex layout.'
        ),
        'success',
        resetState
      );
    ...
    const convert = async () => {
      ...
      if (issue) {
        showAlert(
          translated('pdfToWord.limitTitle', 'Conversion limit'),
          issue,
          'warning'
        );
        return;
      ...
      try {
        ...
        const shouldUseServer = shouldUseServerDocxConversion({
          serverEnabled: serverConfig.enabled,
          fileCount: state.files.length,
          mode,
          fileSize: first.size,
          clientMaxBytes: serverConfig.clientMaxBytes,
          textLayerKind: profile?.kind,
        });
        if (shouldUseServer) {
          if (state.files.length !== 1)
            throw new Error(
              translated(
                'pdfToWord.errors.serverOneFile',
                'Server conversion processes one PDF at a time. Convert large, OCR, or visual PDFs separately.'
              )
            );
        ...
        if (
          ...
          clearServerStatus();
          showAlert(
            translated('pdfToWord.cancelledTitle', 'Conversion cancelled'),
            translated(
              'pdfToWord.errors.cancelled',
              'The conversion was cancelled.'
            ),
        ...
        if (
          ...
          try {
            ...
            clearServerStatus();
            showAlert(
              translated('pdfToWord.errorTitle', 'Conversion failed'),
              knownStructureError(fallbackError)
        ...
        clearServerStatus();
        const message =
          error instanceof Error && error.message === 'UPLOAD_INTERRUPTED'
            ? translated(
                'pdfToWord.errors.uploadInterrupted',
                'The upload connection was interrupted. Please retry once. If it continues, contact the administrator and include the time of the attempt.'
              )
            : error instanceof Error && error.message === 'DOCX_UNAVAILABLE'
              ? translated(
                  'pdfToWord.errors.serverUnavailable',
                  'Server conversion is unavailable. Try a small native-text PDF in the browser or contact the administrator.'
                )
              : knownStructureError(error)
                ? friendlyServerError('INVALID_PDF_STRUCTURE')
                : translated(
                    'pdfToWord.errors.client',
                    'Your browser could not convert this PDF. Try OCR text or visual layout mode.'
                  );
        showAlert(
          translated('pdfToWord.errorTitle', 'Conversion failed'),
          message
    ...
    const selectFiles = (incoming: FileList | null) => {
      ...
      if (tooLarge) {
        showAlert(
          ...
          translated(
            'pdfToWord.errors.fileLimit',
            `PDF to Word currently supports a maximum of ${formatBytes(serverConfig.maxUploadBytes)} per file on this server.`,
            {
              file: tooLarge.name,
              limit: formatBytes(serverConfig.maxUploadBytes),
            }
          )
        );
        return;
    ...
    cancelServerButton?.addEventListener('click', async () => {
      ...
      try {
        if (activeJobId)
          await fetch(`/api/docx/jobs/${activeJobId}`, {
            method: 'DELETE',
            credentials: 'include',
          });
        else if (activeSlotId) {
          await activeUpload?.abort(true).catch((): undefined => undefined);
          await fetch(`/api/docx/upload-slots/${activeSlotId}`, {
            method: 'DELETE',
            credentials: 'include',
          });
        }
