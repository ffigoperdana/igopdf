...
router.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({ success: false, error: 'Catatan penyelesaian tidak valid' });
      return;
    }
    const note = sanitizeRichText(parsed.data.contentHtml);
    if (note.characterCount < MIN_RESOLUTION_CHARACTERS) {
      res.status(400).json({
        success: false,
        error: 'Catatan penyelesaian minimal 50 karakter',
...
router.get(
  ...
  asyncRoute(async (req, res, next) => {
    ...
    const attachment = await getComplaintAttachmentForAdmin(
      ticketId,
      attachmentId
    );
    if (!attachment) {
    ...
    res.download(
      ...
      (error) => {
        if (!error) return;
        logger.error('Admin complaint attachment download failed', {
          ticketId,
          attachmentId,
          code: downloadErrorCode(error),
          reason: error.message,
        });
        // The browser may abort after a download already starts. Avoid routing a
        // second response through Express once headers have been sent.
